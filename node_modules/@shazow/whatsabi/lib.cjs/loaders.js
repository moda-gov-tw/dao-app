...
class SourcifyABILoader {
    async loadABI(address) {
        ...
        try {
            return (await (0, utils_js_1.fetchJSON)("https://repo.sourcify.dev/contracts/full_match/1/" + address + "/metadata.json")).output.abi;
        }
        catch (error) {
            if (error.status !== 404)
                throw error;
        ...
        try {
            return (await (0, utils_js_1.fetchJSON)("https://repo.sourcify.dev/contracts/partial_match/1/" + address + "/metadata.json")).output.abi;
        }
        catch (error) {
            if (error.status !== 404)
                throw error;
