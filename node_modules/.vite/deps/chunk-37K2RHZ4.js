...
function createRouter2(init) {
  ...
  async function handleFetcherAction(key, routeId, path, match, requestMatches, submission) {
    ...
    let existingFetcher = state.fetchers.get(key);
    let fetcher = getSubmittingFetcher2(submission, existingFetcher);
    state.fetchers.set(key, fetcher);
    updateState({
      fetchers: new Map(state.fetchers)
    });
    ...
    if (isRedirectResult2(actionResult)) {
      fetchControllers.delete(key);
      if (pendingNavigationLoadId > originatingLoadId) {
        let doneFetcher = getDoneFetcher2(void 0);
        state.fetchers.set(key, doneFetcher);
        updateState({
          fetchers: new Map(state.fetchers)
        });
        ...
        fetchRedirectIds.add(key);
        let loadingFetcher = getLoadingFetcher2(submission);
        state.fetchers.set(key, loadingFetcher);
        updateState({
          fetchers: new Map(state.fetchers)
        });
  ...
  async function handleFetcherLoader(key, routeId, path, match, matches, submission) {
    let existingFetcher = state.fetchers.get(key);
    let loadingFetcher = getLoadingFetcher2(submission, existingFetcher ? existingFetcher.data : void 0);
    state.fetchers.set(key, loadingFetcher);
    updateState({
      fetchers: new Map(state.fetchers)
    });
    ...
    if (isRedirectResult2(result)) {
      if (pendingNavigationLoadId > originatingLoadId) {
        let doneFetcher2 = getDoneFetcher2(void 0);
        state.fetchers.set(key, doneFetcher2);
        updateState({
          fetchers: new Map(state.fetchers)
        });
  ...
  async function startRedirectNavigation(state2, redirect3, _temp) {
    ...
    let activeSubmission = submission || getSubmissionFromNavigation2(state2.navigation);
    if (redirectPreserveMethodStatusCodes2.has(redirect3.status) && activeSubmission && isMutationMethod2(activeSubmission.formMethod)) {
      ...
      await startNavigation(redirectHistoryAction, redirectLocation, {
        overrideNavigation: getLoadingNavigation2(redirectLocation),
        fetcherSubmission: activeSubmission,
        preventScrollReset: pendingPreventScrollReset
      });
    } else {
      let overrideNavigation = getLoadingNavigation2(redirectLocation, activeSubmission);
      await startNavigation(redirectHistoryAction, redirectLocation, {
        overrideNavigation,
        preventScrollReset: pendingPreventScrollReset
      });
  ...
  function getScrollKey(location, matches) {
    if (getScrollRestorationKey) {
      let key = getScrollRestorationKey(location, matches.map((m) => createUseMatchesMatch2(m, state.loaderData)));
      return key || location.key;
    }
    return location.key;
  }
...
function getMatchesToLoad2(history, state, matches, submission, location, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionData, pendingError) {
  ...
  let navigationMatches = boundaryMatches.filter((match, index) => {
    ...
    return shouldRevalidateLoader2(match, _extends3({
      ...
      actionResult,
      defaultShouldRevalidate: isRevalidationRequired || currentUrl.pathname + currentUrl.search === nextUrl.pathname + nextUrl.search || currentUrl.search !== nextUrl.search || isNewRouteInstance2(currentRouteMatch, nextRouteMatch)
    }));
  });
  let revalidatingFetchers = [];
  fetchLoadMatches.forEach((f, key) => {
...
async function callLoaderOrAction2(type, request, match, matches, manifest, mapRouteProperties2, basename, opts) {
  ...
  try {
    ...
    if (match.route.lazy) {
      if (handler) {
        let values = await Promise.all([runHandler(handler), loadLazyRouteModule2(match.route, mapRouteProperties2, manifest)]);
        result = values[0];
      } else {
...
function getInternalRouterError2(status, _temp4) {
  ...
  return new ErrorResponse2(status || 500, statusText, new Error(errorMessage), true);
}
...
function deserializeErrors(errors) {
  ...
  for (let [key, val] of entries) {
    if (val && val.__type === "RouteErrorResponse") {
      serialized[key] = new ErrorResponse2(val.status, val.statusText, val.data, val.internal === true);
...
var Link = React2.forwardRef(function LinkWithRef(_ref4, ref) {
  ...
  return React2.createElement("a", _extends4({}, rest, {
...
function useSubmitFetcher(fetcherKey, fetcherRouteId) {
  ...
  return React2.useCallback(function(target, options) {
    ...
    !(fetcherRouteId != null) ? true ? invariant2(false, "No routeId available for useFetcher()") : invariant2(false) : void 0;
 ...
 * @license MIT
 */
//# sourceMappingURL=chunk-37K2RHZ4.js.map
