...
var R = { externalWallets() {
  ...
  var t, a;
  const { mobileWallets: o, desktopWallets: r } = C.state, e = (t = R.recentWallet()) == null ? void 0 : t.id, i2 = i.isMobile() ? o : r, l = i2?.filter((d) => e !== d.id);
  return (a = i.isMobile() ? l?.map(({ id: d, name: v, links: x }) => ({ id: d, name: v, mobile: x, links: x })) : l?.map(({ id: d, name: v, links: x }) => ({ id: d, name: v, desktop: x, links: x }))) != null ? a : [];
...
var s = { MOBILE_BREAKPOINT: 600, W3M_RECENT_WALLET_INFO: "W3M_RECENT_WALLET_INFO", EXPLORER_WALLET_URL: "https://explorer.walletconnect.com/?type=wallet", WAGMI_WALLET: "wagmi.wallet", getShadowRootElement(t, a) {
  ...
  return (a = e?.[t]) != null ? a : r && o ? de.getAssetImageUrl(o) : "";
}, getTokenIcon(t) {
  var a, o;
  const r = (a = Wa[t]) == null ? void 0 : a.icon, { projectId: e, tokenImages: i2 } = C.state;
  return (o = i2?.[t]) != null ? o : e && r ? de.getAssetImageUrl(r) : "";
}, isMobileAnimation() {
  ...
  const { pairingUri: a } = P.state, { mobile: o, name: r } = t, e = o?.native, i2 = o?.universal;
  s.setRecentWallet(t);
  function l(d) {
    let v = "";
  ...
  try {
    ...
    await f.client().connectConnector(t, o?.id), pe.close();
  } catch (o) {
  ...
  const { id: t, desktop: a, mobile: o, injected: r } = i.getWalletRouterData(), e = R.installedInjectedWallets(), i2 = Boolean(r?.length), l = e.some((H) => H.id === t), d = Boolean(a?.native), v = Boolean(a?.universal), x = Boolean(o?.native) || Boolean(o?.universal);
...
var ie = class extends LitElement {
  ...
  async preloadListings() {
    ...
    if (C.state.enableExplorer) {
      ...
      const { recomendedWallets: o } = de.state, r = R.installedInjectedWallets(), e = (t = a?.map((d) => s.getChainIcon(d.id))) != null ? t : [], i2 = o.map((d) => s.getWalletIcon(d)), l = r.map((d) => s.getWalletIcon(d));
...
var le = class extends LitElement {
  constructor() {
    ...
    this.activeChainId = t?.id, this.unwatchNetwork = f.client().watchNetwork((a) => {
      const o = a.chain;
...
var Ne = class extends LitElement {
  ...
  async preloadThemeImages() {
    try {
      const { themeVariables: t } = ue.state, a = [t?.["--w3m-background-image-url"], t?.["--w3m-logo-image-url"]].filter(Boolean);
  ...
  async connectAndWait() {
    ...
    if (clearTimeout(this.timeout), !this.isAccountConnected && t) {
      ...
      try {
        ...
        await f.client().connectWalletConnect((o) => P.setPairingUri(o), a?.id);
      } catch (a) {
...
var It = class extends LitElement {
  constructor() {
    ...
    this.chainId = t?.id, this.label = t?.name, this.unsubscribeNetwork = b.subscribe(({ selectedChain: a }) => {
  ...
  render() {
    const { chains: t, selectedChain: a } = b.state, o = t?.map((i2) => i2.id), r = a && o?.includes(a.id), e = t && t.length <= 1 && r;
...
var pt = class extends LitElement {
  ...
  onSetChainData(t) {
    if (t) {
      const { chains: a } = b.state, o = a?.map((r) => r.id);
...
var zt = class extends LitElement {
  ...
  onFormatAndRedirect(t) {
    const { desktop: a, name: o } = i.getWalletRouterData(), r = a?.native;
    if (r) {
...
var Ft = class extends LitElement {
  ...
  onFormatAndRedirect(t, a = false) {
    const { mobile: o, name: r } = i.getWalletRouterData(), e = o?.native, i2 = o?.universal;
    if (e && !a) {
  ...
  render() {
    const { name: t, id: a, image_id: o, app: r, mobile: e } = i.getWalletRouterData(), { isWeb: i2 } = s.getCachedRouterWalletPlatforms(), l = r?.ios, d = e?.universal;
...
var Mt = class extends LitElement {
  ...
  async onSelectChain(t) {
    try {
      ...
      r ? a?.id === t.id ? W.reset("Account") : s.getWagmiWalletType() === '"walletConnect"' ? (await f.client().switchNetwork({ chainId: t.id }), W.reset("Account")) : W.push("SwitchNetwork", { SwitchNetwork: t }) : o ? (b.setSelectedChain(t), pe.close()) : (b.setSelectedChain(t), W.push("ConnectWallet"));
    } catch (a) {
...
var Kt = class extends LitElement {
  ...
  onFormatAndRedirect(t) {
    const { desktop: a, name: o } = i.getWalletRouterData(), r = a?.universal;
    if (r) {
...
//# sourceMappingURL=dist-BIDPNPKM.js.map
